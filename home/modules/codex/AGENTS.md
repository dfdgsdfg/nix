...
2. Delegate narrow, targeted read-only lookups to `scout`, broader codebase
   exploration to `explorer`, and bounded implementation, focused test
   execution, or failure analysis to `worker`.
3. Use `scout` to locate specific files, symbols, ownership, or one bounded
